...
var _required_power_factor: float
var _pitch: float
...
func _calculate(velocity: float, forward: Vector3) -> void:
	...
	thrust = _thrust_factor * density * pow(rps, 2.0) * pow(diameter, 4.0)
	var power_required := _required_power_factor * pow(maxf(0.01, absf(rps)), 3.0) * pow(diameter, 5.0) * density
