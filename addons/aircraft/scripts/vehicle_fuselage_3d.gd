...
@export var forward_width := 0.25:
	set(value):
		forward_width = value
		_dirty = true
		update_gizmos()

@export var backward_width := 0.1:
	set(value):
		backward_width = value
		_dirty = true
		update_gizmos()

@export var center_width := 1.0:
	set(value):
		center_width = value
		_dirty = true
		update_gizmos()

@export_range(0.0, 1.0) var center_position := 0.25:
	set(value):
...
func _build_sections() -> void:
	...
	if center_position > 0.0 and center_position < 1.0:
		var center := forward * length * (0.5 - center_position)
		_build_part(center, forward_point, center_width, forward_width)
		_build_part(center, backward_point, center_width, backward_width)
	else:
		_build_part(backward_point, forward_point, backward_width, forward_width)
...
func _get_nominal_section_length(width_base: float, tip_width: float, part_length: float) -> float:
	var max_width := maxf(width_base, maxf(tip_width, center_width))
