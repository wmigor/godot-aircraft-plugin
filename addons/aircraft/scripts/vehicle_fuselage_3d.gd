...
## Destabilizing pitch moment
@export var pitch_max := 0.5
## Air density.
...
## Мanual calculation of the force and torque generated by the fuselage. For example, to plot graphs.
...
func _calculate_section_forces(section: Section, wind: Vector3) -> void:
	...
	var drag_factor := zero_angle_drag + absf(sin(angle_of_attack)) * drag_max
	var pressure := 0.5 * wind.dot(wind_direction) * wind.length() * area * density
	var force := drag_factor * pressure * wind_direction
	section.force += (force - section.force) * 0.5
	var right := section.global_transform.basis.x
	section.torque = right * pitch_max * sin(2.0 * angle_of_attack) * pressure
