...
func update_factors(data: Data) -> void:
	data.lift_factor = _get_lift(data.angle_of_attack)
	data.lift_factor += _get_control_surface_lift(data)
	data.drag_factor = _get_drag(data.angle_of_attack)
	data.pitch_factor = _get_pitch(data.angle_of_attack)
...
func _get_pitch(_alpha: float) -> float:
	return 0.0


func _get_control_surface_lift(data: Data) -> float:
	if data.angle_of_attack <= -stall_angle or data.angle_of_attack >= stall_angle:
		return 0.0
	return lift_slope * data.control_surface_angle * data.control_surface_fraction * 0.5
