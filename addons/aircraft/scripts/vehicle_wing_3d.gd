...
var _root_wing: VehicleWing3D
var _parent_wing: VehicleWing3D
...
func _enter_tree() -> void:
	_parent_wing = get_parent() as VehicleWing3D
	if _parent_wing != null:
		_root_wing = _parent_wing._root_wing
		_body = _root_wing._body
	else:
		_root_wing = self
		_body = get_parent() as RigidBody3D
	_root_wing._total_wings.append(self)
	_set_dirty()
	_try_rebuild()
...
func _exit_tree() -> void:
	...
	_parent_wing = null
	if _root_wing != null:
		_root_wing._total_wings.erase(self)
	_root_wing = null
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if parent is not RigidBody3D and parent is not VehicleWing3D:
		warnings.append("VehicleWing3D serves to provide a aerodynamic to a VehicleBody3D or RigidBody3D. Please use it as a child of a VehicleBody3D or RigidBody3D or VehicleWing3D.")
	return warnings
...
func _update_gizmo_total_wings() -> void:
	if _root_wing != null:
		for wing in _root_wing._total_wings:
			wing.update_gizmos()


func _update_child_wing() -> void:
	for i in get_child_count():
		var wing := get_child(i) as VehicleWing3D
		if wing == null:
			continue
		var tip := get_tip()
		wing.position.z = tip.z
		wing.position.y = tip.y
		wing.rotation.x = twist
		wing.chord = chord * taper
		wing.offset = span * (0.5 if mirror else 1.0)
		wing.mirror = mirror
...
func _update_aspect_ratio() -> void:
	...
	for wing in _root_wing._total_wings:
		...
		total_area += area
		if wing.span > total_span:
			total_span = wing.span
	if absf(total_area) < 0.001:
		_aspect_ratio = 1.0
	else:
		_aspect_ratio = total_span * total_span / total_area
	if absf(_aspect_ratio) < 0.001:
		_aspect_ratio = 1.0
	for wing in _root_wing._total_wings:
		wing._aspect_ratio = _aspect_ratio
...
func _update_lift_geometry_factor() -> void:
	if _root_wing == null or len(_root_wing._total_wings) <= 0 or not _root_wing.mirror:
		return
	var total_span := span
	for wing in _root_wing._total_wings:
		if wing.span > total_span:
	...
	for wing in _root_wing._total_wings:
		for section in wing._sections:
			var x := section.transform.origin.x
			section.correct_lift_geometry_factor = sqrt(1.0 - pow(absf(x) / (total_span / 2.0), 2.0))
...
func _create_wing_section(p_control_surface: ControlSurface, p_position: Vector3, p_chord: float, p_length: float, p_twist: float, is_mirror: bool) -> Section:
	...
	section.oswald_factor = oswald_factor
	section.stall = false
...
func get_mac_forward_position_local() -> float:
	...
	pos += tan_sweep_le * get_mac_right_position()
